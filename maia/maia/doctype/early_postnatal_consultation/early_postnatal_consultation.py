...
class EarlyPostnatalConsultation(Document):
...
def get_last_weight(consultation, pregnancy, child):
    ...
    ep_weights = frappe.get_all("Early Postnatal Consultation", filters={"pregnancy_folder": pregnancy}, fields=["name", "consultation_date", weight_field])
    ...
    for ep_weight in ep_weights:
        if ep_weight[weight_field] is not None and ep_weight[weight_field]!=0 and isinstance(ep_weight[weight_field], int) and ep_weight.name != consultation:
