...
def execute():
	...
	for cervical_smear in cervical_smears:
		...
		if doc.parenttype != "Patient Record":
			...
			try:
				frappe.db.sql("""update `tabCervical Smear` set parenttype='Patient Record', parent='{0}', parenfield='cervical_smear_table' where name='{1}'""".format(patient_record, doc.name))
	...
	for patient_record in patient_records:
		doc = frappe.get_doc("Patient Record", patient_record.name)

		if doc.cervical_smear is not None:

			doc.append('cervical_smear_table', {
				'result': doc.cervical_smear
			})
